...
const CodeRenderer: React.FC<CodeRendererProps> = ({ file, isPreview }) => {
  ...
  const highlightLine = (line: string) => {
    ...
    // Handle syntax highlighting based on file type
    if (file.type === 'json') {
      processed = processed
  ...
  if (isPreview && file.type === 'markdown') {
    const previewContent = file.content
      ...
      .map((line, i) => {
        ...
        if (trimmed.startsWith('# ')) return `<h1 key=${i} class="text-3xl font-bold mb-4 mt-8 pb-2 border-b border-theme">${highlighted.replace('# ', '')}</h1>`;
        if (trimmed.startsWith('## ')) return `<h2 key=${i} class="text-2xl font-bold mb-3 mt-6">${highlighted.replace('## ', '')}</h2>`;
        if (trimmed.startsWith('### ')) return `<h3 key=${i} class="text-xl font-semibold mb-2 mt-4 text-primary">${highlighted.replace('### ', '')}</h3>`;
        if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
          return `<li key=${i} class="mb-1 opacity-90 list-none flex gap-2"><span class="text-primary">•</span> <span>${highlighted.substring(2)}</span></li>`;
        }
        if (trimmed === '---') return `<hr key=${i} class="my-8 border-theme" />`;
        if (!trimmed) return '<div class="h-4"></div>';
        
        return `<p key=${i} class="mb-4 leading-relaxed opacity-80">${highlighted}</p>`;
      })
      .join('');

    return (
      <div className="flex-1 overflow-y-auto bg-background-dark p-8 md:p-12 font-display scroll-smooth custom-scrollbar transition-colors duration-300">
        <div 
  ...
  return (
    <div 
      className="flex-1 overflow-y-auto flex relative scroll-smooth custom-scrollbar" 
    ...
    >
      ...
      {/* Mini-map */}
      <div className="w-24 border-l border-theme/10 opacity-10 hidden lg:block select-none overflow-hidden flex-shrink-0 sticky top-0 h-full">
